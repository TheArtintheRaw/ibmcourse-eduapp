...
class Question(models.Model):
    ...
    def is_answer_correct(self, selected_ids):
        correct_choices = self.choices.filter(
            id__in=selected_ids, is_correct=True
        ).count()
        return correct_choices == self.choices.filter(is_correct=True).count()
...
class Submission(models.Model):
    ...
    selected_choices = models.ManyToManyField(Choice, related_name="submissions")

    def __str__(self):
        return f"Submission id: {self.pk} for Enrollment id: {self.enrollment.pk}"
